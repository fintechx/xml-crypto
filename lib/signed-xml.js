...
function SignedXml(idMode, options) {
  ...
  this.signatureAlgorithm = this.options.signatureAlgorithm || "http://www.w3.org/2001/04/xmldsig-more#rsa-sha512";
 ...
 *
 */
SignedXml.prototype.computeSignature = function(xml, opts, inclusiveNamespacesPrefixList, callback) {
  ...
  signatureXml += this.createSignedInfo(doc, prefix, inclusiveNamespacesPrefixList);
    ...
    for (var h in nodes) {
      ...
      for (var t in ref.transforms) {
        ...
        if (!ref.inclusiveNamespacesPrefixList) {
          ...
          res += "<" + prefix + "Transform Algorithm=\"" + transform.getAlgorithmName() + "\" >"
          const prefixList = ref.inclusiveNamespacesPrefixList.join(' ');
          res += '<ec:InclusiveNamespaces xmlns:ec="http://www.w3.org/2001/10/xml-exc-c14n#" PrefixList="' + prefixList  + '" />';
 ...
 *
 */
SignedXml.prototype.createSignedInfo = function(doc, prefix, inclusiveNamespacesPrefixList) {
  ...
  if (!inclusiveNamespacesPrefixList) {
    ...
    res += "<" + currentPrefix + "CanonicalizationMethod Algorithm=\"" + transform.getAlgorithmName() + "\" >"
    const prefixList = inclusiveNamespacesPrefixList.join(' ');
    res += '<ec:InclusiveNamespaces xmlns:ec="http://www.w3.org/2001/10/xml-exc-c14n#" PrefixList="' + prefixList  + '" />';
